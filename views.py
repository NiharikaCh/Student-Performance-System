...
def mach(request):
	...
	if request.POST.get('rollno') and request.POST.get('achievement'):
		...
		return render(request, 'student/add_achievements.html',{'li':li,'lp':lp})  
	#return render(request,'student/add_achievements.html',{'li':li,'lp':lp})
#open marks upload,edit,view page
def subject(request):
	if request.method == 'POST':
		...
		fc=cursor.fetchall()
		return render(request,'student/course.html',{'w':w,'li':li,'lp':lp,'x':x,'fc':fc[0][0]})
	return render(request,'student/student.html')
...
def login1(request):
	if request.method == 'POST':
		...
		if len(objects)!=0 and p[0]=='2':
			return render(request,'student/student.html',{'p':p,'lp':q})
...
def addgradeschema(request):
	if request.method == 'POST':
		...
		cursor.execute("insert into student_gradeschema(A,A2,B,B2,C,C2,D,Semester,Facultycourse_Id_id) values('%s','%s','%s','%s','%s','%s','%s','%s','%s')"%(x,y,z,e,f,q,r,sem,fcid))
...
def func(request):
	if request.method == 'POST':
		...
		if len(r)!=0:
			...
			return render(request, 'student/Acad.html',{'p':p,'v':v1[0][0],'s':s,'lp':lp})
...
def emarks(request):
	if request.method == 'POST':
		print(float(request.POST.get('ns1')))
...
def editm(request):
	...
	if now.month in a1:
		...
		q='M_'+y
	cursor.execute("select Student_Id_id,Marks,Marks_perc from student_academic_score where semester='%s' and Academic_Course_Id_id='%s' and Exam_Type='%s'"%(q,w,exty))
	t=cursor.fetchall()
...
def ordermarks(request):
	if request.method == 'POST':
		...
		for i in b:
			c.append(list(i))
		cursor.execute("select Student_Id_id,Exam_Type,Marks,Marks_perc,Semester,Facultycourse_Id_id from student_academic_score order by Marks desc")
		d=cursor.fetchall()
		e=[]
		f=[]
		f1=[]
...
time=now1.hour
print(time)
#results = Academic_score.function()  
#for i in results:
